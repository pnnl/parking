...
import { cloneDeep, flatten, get, has, isUndefined, merge, range, set, uniqBy } from "lodash";
...
const main = async () => {
  try {
    const dir = resolve(process.cwd(), process.env.SEEDER_DATA_PATH ?? "data/", "alloc/");
    ...
    let items = await Promise.all(
      files
        ...
        .map(async (file) => {
          ...
          const records = spaces.map((space: any) => space);
          logger.info(`Loaded ${records.length} records of curb alloc data from: ${file}`);
          return records;
        })
    );
    items = flatten(items);
    ...
    Object.values(
      data.reduce((p, c) => {
        ...
        return p;
      }, {} as Record<string, (typeof instance)[]>)
    ).forEach((v) => {
      v = uniqBy(v, "label");
      v.forEach((s, i) => {
    ...
    logger.info(`Added ${occupancies.length} record${occupancies.length === 1 ? "" : "s"} to the occupancy table.`);
  } catch (error: any) {
    logger.warn(error);
  }
};
